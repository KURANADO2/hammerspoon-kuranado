...
簡体字拼音とひらがなの入力方法を切り替えるには、ショートカットキー `⌥` `L` を使用します
...
`⇪` キーを使用して簡体字拼音と ABC を切り替えることは問題ありませんが、簡体字拼音と日本語入力方式を切り替えることは、常に私の最大の悪夢でした。 システム独自のスイッチングショートカット `⌃` `space` を使用します。

<div align='center'>
  <img src='./images/input-method3.png' width='200'/>
</div>

1つ目：ポップアップボックスに一定の遅延が発生します（耐えられません）。2つ目：すべての入力方法を順番に切り替えるには、 `⌃` `space` を繰り返し押す必要があります（常に切り取ってから、もう一度向きを変えます） 。 **同じアプリ内で**これを1日に何百回も実行しなければならないのは恐ろしいことです。

ここで私は切り替えソリューションを提供します。他の方がより合理的なソリューションを持っている場合、彼らは提案することを歓迎します：

現在の入力方法|ショートカットキー|切り替え
-|-|-
ABC/日本語入力方式|`⌥` `L`|簡体字拼音
簡体字拼音|`⌥` `L`|日本語入力方式
簡体字拼音|`⇪`|ABC
...
**注：** アプリケーションに応じて入力方式の自動切り替えを使用する人も多くいます。たとえば、IDEA　に切り替えると、入力方式は自動的にABCに切り替わり、WeChat　に入ると入力が行われます。メソッドは自動的に簡体字拼音に切り替えられます...、これは確かに優れたソリューションです。もちろん、Hammerspoon　も実装できますが、このソリューションは私には適していません。
